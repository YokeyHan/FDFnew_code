...
class PerformanceMetrics:
    ...
    perplexity_loss: float = 0.0

    vqloss, recon_loss, quant_loss, sparsity_loss, perplexity_loss

    def __repr__(self):
        return (f"MSE Loss: {self.mse_loss:.6f}, MAE Loss: {self.mae_loss:.6f}, "
                f"Total_loss: {self.loss:.6f}, VQ_Loss: {self.vqloss:.6f}, "
                f"recon_loss: {self.recon_loss:.6f}, quant_loss: {self.quant_loss:.6f}, "
                f"sparsity_loss: {self.sparsity_loss:.6f}, perplexity_loss: {self.perplexity_loss:.6f}")
...
class ModelTrainer:
    ...
    def _train_one_epoch(self):
        ...
        for x, y in tqdm(self.train_loader, desc="Training", disable=not self.args.use_tqdm):
            ...
            predictions, vqloss,recon_loss, quant_loss, sparsity_loss, perplexity_loss = self.model(x, task="train")
            mse_loss = self.mse_criterion(predictions, y)
            mae_loss = self.mae_criterion(predictions, y)
            loss = mae_loss + vqloss
            loss.backward()
            ...
            metrics.vqloss += vqloss.item()
            metrics.recon_loss += recon_loss.item()
            metrics.quant_loss += quant_loss.item()
            metrics.sparsity_loss += sparsity_loss.item()
            metrics.perplexity_loss += perplexity_loss.item()
        ...
        metrics.vqloss /= len(self.train_loader)
        metrics.recon_loss /= len(self.train_loader)
        metrics.quant_loss /= len(self.train_loader)
        metrics.sparsity_loss /= len(self.train_loader)
        metrics.perplexity_loss /= len(self.train_loader)
    ...
    def _validate_one_epoch(self):
        ...
        for x, y in tqdm(self.val_loader, desc="Validation", disable=not self.args.use_tqdm):
            ...
            predictions, vqloss,recon_loss, quant_loss, sparsity_loss, perplexity_loss= self.model(x, task="valid")
            mse_loss = self.mse_criterion(predictions, y)
            mae_loss = self.mae_criterion(predictions, y)
            loss = mae_loss + vqloss
        ...
        metrics.vqloss /= len(self.val_loader)
        metrics.recon_loss /= len(self.val_loader)
        metrics.quant_loss /= len(self.val_loader)
        metrics.sparsity_loss /= len(self.val_loader)
        metrics.perplexity_loss /= len(self.val_loader)
    ...
    def evaluate_test(self):
        ...
        for x, y in tqdm(self.test_loader, desc="Testing", disable=not self.args.use_tqdm):
            if self.args.loss_type == 'mse':
                predictions, vqloss,recon_loss, quant_loss, sparsity_loss, perplexity_loss = self.model(x, task="test")
                mse_loss = self.mse_criterion(predictions, y)
                mae_loss = self.mae_criterion(predictions, y)
                loss = mae_loss + vqloss
            ...
            metrics.perplexity_loss += perplexity_loss.item()
        n = self.test_loader
        metrics.mse_loss /= len(n)
        metrics.mae_loss /= len(n)
        metrics.loss /= len(n)
        metrics.vqloss /= len(n)
        metrics.recon_loss /= len(n)
        metrics.quant_loss /= len(n)
        metrics.sparsity_loss /= len(n)
        metrics.perplexity_loss /= len(n)
