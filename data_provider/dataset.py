...
class STDataset(Dataset):
    ...
    def __read_data(self):
        #file_path = Path(self.args.data_dir) / self.file_name / "{train}.npz"

        #_data = pd.read_csv(file_path, index_col="date", parse_dates=True)
        _data = pd.read_hdf("/home/user/plh/Prediction/FDF-main/FDF-main/datasets/METR-LA/metr-la.h5")
        #_data = pd.read_hdf("/home/user/plh/Prediction/FDF-main/FDF-main/datasets/PEMS-BAY/pems-bay.h5")
        train_samples_len = int(len(_data) * 0.7)
        test_samples_len = int(len(_data) * 0.2)
        val_samples_len = len(_data) - train_samples_len - test_samples_len
        self.slice_map = {
...
class mySTDataset(Dataset):
    ...
    def __read_data(self):
        ...
        #_data = pd.read_csv(file_path, index_col="date", parse_dates=True)
        _data = pd.DataFrame(np.load('/home/user/plh/Prediction/FDF-main/FDF-main/datasets/Xian/data_xianCO.npy'))
        train_samples_len = int(len(_data) * 0.7)
        test_samples_len = int(len(_data) * 0.2)
        val_samples_len = len(_data) - train_samples_len - test_samples_len
        self.slice_map = {
